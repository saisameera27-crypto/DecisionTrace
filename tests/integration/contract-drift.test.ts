...
function validateRecordedResponse(stepName: string, schema: z.ZodTypeAny): {
  ...
  try {
    ...
    const result = validateWithSchema(schema, response);

    if (result.success) {
      return {
        valid: true,
        schemaVersion: response.schemaVersion || CURRENT_SCHEMA_VERSION,
      };
    } else {
      return {
        valid: false,
...
function checkSchemaVersion(recordedData: any, expectedVersion: string): {
  ...
  const recordedVersion = recordedData.schemaVersion || 'unknown';
  return {
  ...
  describe('Schema Version Matching', () => {
    it('should fail if recorded response schema version does not match current', () => {
      ...
      if (!versionCheck.matches) {
        ...
        expect(versionCheck.matches).toBe(true);
      }
    });
    ...
    it('should require schema version in recorded responses', () => {
      ...
      const version = step1Response.schemaVersion || CURRENT_SCHEMA_VERSION;
